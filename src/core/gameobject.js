...
export class GameObject extends Phaser.GameObjects.Container
{
    ...
    { 
        ...
        // 4) 從場景中移除 container
        this.removeAll(true)    // true -> destroy 子物件
        this.destroy(true);     // destroy self

        // 5) 移除場景引用，讓 GC 可以回收，要放(4)之後，不然切換場景時會當機
        this.scene = null;
    }
    ...
    {
        const{bb}=this.ctx;
        if(bb.tile.tw>1||bb.tile.th>1)
        {
            this.removeWeight?.()
            const w=this.scene.map.getWeight(pt,bb.tile);
            this.addWeight?.()
            return w;
        }
        else
