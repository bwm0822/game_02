...
- **Quest/Dialog**：資料為 `public/assets/json/quest_v2.json`/`dialog_v2.json`（由 `xls/*_v2.xlsx` 透過 `node scripts/quest_v2.js`/`scripts/dialog_v2.js` 產生，非 npm script）；對話元件只用 `com_talk_v2.js`（`com_talk.js` 是死代碼）；修改任務或對話前請先參考 [docs/quest-dialog-architecture.md](docs/quest-dialog-architecture.md)
